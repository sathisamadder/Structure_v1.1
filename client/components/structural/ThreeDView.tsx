  ...
  return (
    <div className="relative h-[520px] w-full rounded-md border bg-white">
      <Canvas shadows camera={{ position: [10, 10, 10], fov: 60 }}>
        ...
        <Suspense fallback={null}>
          <group position={[-nodes.offsetX, 0, -nodes.offsetY]}>
            {showSlabs && nodes.slabs.map((s, idx)=> (
              <mesh key={"slab-"+idx} rotation={[-Math.PI/2,0,0]} position={[s.cx, s.y, s.cy]} receiveShadow>
            ...
            {nodes.beams.map((b, idx)=> (
              <mesh key={idx} position={[b.cx, b.cy, b.cz]} rotation={[0, b.ry, 0]} castShadow>
            ...
            {showGrid && (<gridHelper args={[100, 100, "#cbd5e1", "#e2e8f0"]} position={[0,0.001,0]} />)}
      ...
      <div className="absolute right-2 top-2 flex gap-2 rounded-md border bg-white/80 p-2 text-xs">
        <button className="px-2 py-1 rounded border" onClick={()=>setPreset("iso")}>ISO</button>
        <button className="px-2 py-1 rounded border" onClick={()=>setPreset("top")}>Top</button>
        <button className="px-2 py-1 rounded border" onClick={()=>setPreset("front")}>Front</button>
        <button className="px-2 py-1 rounded border" onClick={()=>setShowGrid(s=>!s)}>{showGrid?"Grid on":"Grid off"}</button>
        <button className="px-2 py-1 rounded border" onClick={()=>setShowAxes(s=>!s)}>{showAxes?"Axes on":"Axes off"}</button>
        <button className="px-2 py-1 rounded border" onClick={()=>setShowSlabs(s=>!s)}>{showSlabs?"Slabs on":"Slabs off"}</button>
        <ScreenshotButton />
      </div>
    </div>
  );
...
function buildNodes(grid: GridSpec, building: BuildingSpec){
  ...
  const lx = xs[xs.length-1]; const ly = ys[ys.length-1];
