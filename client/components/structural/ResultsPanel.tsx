            ...
            <tbody>
              {results.columns.map((c, idx) => (
                <tr key={idx} className="border-b">
                  ...
                  <Td>
                    {Math.round(c.suggestedSizeM.b * 1000)}×
                    {Math.round(c.suggestedSizeM.h * 1000)} mm
                  </Td>
                  <Td>
                    {c.bars.count}−Ø{c.bars.diameterMM}
                  </Td>
            ...
            <tbody>
              {results.beams.map((b, idx) => (
                <tr key={idx} className="border-b">
                  ...
                  <Td>
                    {b.suggestedBars.count}−Ø{b.suggestedBars.diameterMM}
                  </Td>
                </tr>
              ))}
            </tbody>
            ...
            <tbody>
              {results.slabs.map((s, idx) => (
                <tr key={idx} className="border-b">
                  ...
                  <Td>
                    Ø{s.bar.diameterMM} @ {s.bar.spacingMM} mm
                  </Td>
                </tr>
              ))}
            </tbody>
            ...
            <tbody>
              {results.footings.map((f, idx) => (
                <tr key={idx} className="border-b">
                  ...
                  <Td>
                    {round(f.suggestedSizeM.b, 2)} ×{" "}
                    {round(f.suggestedSizeM.b, 2)} ×{" "}
                    {round(f.suggestedSizeM.h, 2)} m
                  </Td>
        ...
        {results.bnbc ? (
          <div className="space-y-2">
            <div className="text-xs">
              T = {results.bnbc.periodT}s, W = {results.bnbc.seismicWeightKN}{" "}
              kN, C<sub>s</sub> = {results.bnbc.Cs}, V ={" "}
              {results.bnbc.baseShearKN} kN
            </div>
            <div className="overflow-auto">
              <table className="w-full text-sm">
                <thead>
          ...
          <div className="text-sm text-muted-foreground">
            Add seismic inputs in the left panel to compute BNBC 2020 base
            shear.
          </div>
        )}
      </Section>
...
function Th({
  ...
  return (
    <th className={"px-2 py-2 text-xs font-medium " + (className || "")}>
      {children}
    </th>
  );
}
