...
export default function GridEditor({ grid, onGridChange }: Props) {
  ...
  const widthM = useMemo(
    () => grid.xSpacingsM.reduce((a, b) => a + b, 0),
    [grid],
  );
  const heightM = useMemo(
    () => grid.ySpacingsM.reduce((a, b) => a + b, 0),
    [grid],
  );
  ...
  const updateSpacing = (axis: "x" | "y", idx: number, val: number) => {
    ...
    if (axis === "x") {
      ...
      x[idx] = val;
      onGridChange({ ...grid, xSpacingsM: x });
    } else {
      const y = grid.ySpacingsM.slice();
      y[idx] = val;
      onGridChange({ ...grid, ySpacingsM: y });
    }
  };
  ...
  const updateLabel = (axis: "x" | "y", idx: number, value: string) => {
    if (axis === "x") {
      const labels = (
        grid.xLabels ||
        Array.from({ length: grid.nx + 1 }, (_, i) => String(i + 1))
      ).slice();
      labels[idx] = value;
      ...
      const labels = (
        grid.yLabels ||
        Array.from({ length: grid.ny + 1 }, (_, i) =>
  ...
  const addBay = (axis: "x" | "y") => {
    if (axis === "x") {
      const x = grid.xSpacingsM.concat(
        grid.xSpacingsM[grid.xSpacingsM.length - 1] || 5,
      );
      const cols = Array.from({ length: x.length + 1 }, (_, i) =>
        Array.from(
          { length: grid.ny + 1 },
          (_, j) => grid.columns[Math.min(i, grid.nx)]?.[j] ?? true,
        ),
      );
      const xLabels = (
        grid.xLabels ||
        Array.from({ length: grid.nx + 1 }, (_, i) => String(i + 1))
      ).concat(String(grid.nx + 2));
      onGridChange({
        ...grid,
        nx: grid.nx + 1,
        xSpacingsM: x,
      ...
      const y = grid.ySpacingsM.concat(
        grid.ySpacingsM[grid.ySpacingsM.length - 1] || 4,
      );
      const cols = Array.from({ length: grid.nx + 1 }, (_, i) =>
        Array.from(
          { length: y.length + 1 },
          (_, j) => grid.columns[i]?.[Math.min(j, grid.ny)] ?? true,
        ),
      );
      const yLabels = (
        grid.yLabels ||
      ...
      ).concat(String.fromCharCode(65 + grid.ny + 1));
      onGridChange({
        ...grid,
        ny: grid.ny + 1,
        ySpacingsM: y,
  ...
  const analyzeLayout = async () => {
    ...
    try {
      ...
      const res = await fetch("/api/ai/parse-layout", {
        method: "POST",
    ...
    <div className="space-y-3">
      <div className="flex items-center justify-between gap-2">
        <div className="text-sm text-muted-foreground">
          Total: {round(widthM, 2)}m × {round(heightM, 2)}m
        </div>
        <div className="flex items-center gap-2">
          ...
          <Button
            ...
            disabled={analyzing}
          >
            {analyzing ? "Analyzing..." : "Analyze layout (AI)"}
          </Button>
            ...
            {grid.xSpacingsM.map((v, i) => (
              <div key={i} className="flex items-center gap-2">
                <Input
                  ...
                  value={v}
                  onChange={(e) =>
                    updateSpacing("x", i, parseFloat(e.target.value))
                  }
                  className="w-24"
                />
                <Input
                  type="text"
                  value={(grid.xLabels || [])[i] || String(i + 1)}
                  onChange={(e) => updateLabel("x", i, e.target.value)}
                  className="w-20"
                />
              </div>
            ))}
            <div className="flex gap-2">
              <Button size="sm" onClick={() => addBay("x")}>
                Add X bay
              </Button>
              <Button
                size="sm"
          ...
          {grid.ySpacingsM.map((v, i) => (
            <div key={i} className="flex items-center gap-2">
              <Input
                ...
                value={v}
                onChange={(e) =>
                  updateSpacing("y", i, parseFloat(e.target.value))
                }
                className="w-24"
              />
              <Input
                type="text"
                value={(grid.yLabels || [])[i] || String.fromCharCode(65 + i)}
                onChange={(e) => updateLabel("y", i, e.target.value)}
                className="w-20"
              />
            </div>
          ))}
          <Button size="sm" onClick={() => addBay("y")}>
            Add Y bay
          </Button>
          <Button size="sm" variant="outline" onClick={() => removeBay("y")}>
            Remove Y bay
          </Button>
        </div>
      </div>
  ...
  function QuickGenerators({
    ...
    const apply = (axis: "x" | "y", bays: number, spacing: number) => {
      ...
      if (axis === "x") {
        ...
        const ny = bays;
        const columns = rebuildColumns(nx, ny, grid.columns);
        const yLabels = Array.from({ length: ny + 1 }, (_, i) =>
          String.fromCharCode(65 + i),
        );
    ...
    const allCols = (on: boolean) => {
      const cols = Array.from({ length: grid.nx + 1 }, (_, i) =>
        Array.from({ length: grid.ny + 1 }, (_, j) => on),
      );
        ...
        <div className="flex flex-wrap items-end gap-2 text-xs">
          <div className="flex items-center gap-2">
            ...
            <Input
              ...
              min={1}
              value={xBays}
              onChange={(e) => setXBays(parseInt(e.target.value || "1"))}
            />
            <span>bays</span>
            <Input
              className="w-20"
              ...
              step="0.1"
              value={xSpacing}
              onChange={(e) => setXSpacing(parseFloat(e.target.value || "0"))}
            />
            <span>m spacing</span>
            <Button size="sm" onClick={() => apply("x", xBays, xSpacing)}>
              Apply
            </Button>
          </div>
          <div className="flex items-center gap-2">
            ...
            <Input
              ...
              min={1}
              value={yBays}
              onChange={(e) => setYBays(parseInt(e.target.value || "1"))}
            />
            <span>bays</span>
            <Input
              className="w-20"
              ...
              step="0.1"
              value={ySpacing}
              onChange={(e) => setYSpacing(parseFloat(e.target.value || "0"))}
            />
            <span>m spacing</span>
            <Button size="sm" onClick={() => apply("y", yBays, ySpacing)}>
              Apply
            </Button>
          </div>
          <div className="ml-auto flex items-center gap-2">
            <Button size="sm" variant="outline" onClick={() => allCols(true)}>
              Enable all columns
            </Button>
            <Button size="sm" variant="outline" onClick={() => allCols(false)}>
              Disable all columns
            </Button>
          </div>
        </div>
  ...
  function rebuildColumns(nx: number, ny: number, prev: boolean[][]) {
    const cols: boolean[][] = Array.from({ length: nx + 1 }, (_, i) =>
      Array.from({ length: ny + 1 }, (_, j) => true),
    );
...
function SvgGrid({
  ...
  for (let i = 0; i < grid.xSpacingsM.length; i++)
    xs.push(xs[i] + grid.xSpacingsM[i] * scale);
  const ys: number[] = [padding];
  for (let j = 0; j < grid.ySpacingsM.length; j++)
    ys.push(ys[j] + grid.ySpacingsM[j] * scale);
      ...
      {ys.map((y, j) => (
        <g key={"hyg" + j}>
          ...
          <text
            ...
            fill="#334155"
          >
            {(grid.yLabels || [])[j] || String.fromCharCode(65 + j)}
          </text>
        </g>
      ))}
      {/* columns */}
