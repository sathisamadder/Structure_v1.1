...
function tributaryAreaAt(grid: GridSpec, i: number, j: number): number {
  ...
  const xRight =
    i === grid.nx ? grid.xSpacingsM[grid.nx - 1] / 2 : grid.xSpacingsM[i] / 2;
  const yDown = j === 0 ? grid.ySpacingsM[0] / 2 : grid.ySpacingsM[j - 1] / 2;
  const yUp =
    j === grid.ny ? grid.ySpacingsM[grid.ny - 1] / 2 : grid.ySpacingsM[j] / 2;
...
function pickBars(requiredAreaMM2: number): {
  diameterMM: number;
  count: number;
} {
...
function pickColumnSize(
  ...
  for (const size of sizesMM) {
    const b = size / 1000,
      h = b; // square columns for simplicity
...
function beamFlexure(
  ...
  Mu_kNm: number;
  As_req_MM2: number;
  suggestion: { diameterMM: number; count: number };
} {
...
function slabFlexure(
  spanM: number,
  wULS_KPa: number,
  thicknessM: number,
  fyMPa: number,
): { Mu_kNm_per_m: number; bar: { diameterMM: number; spacingMM: number } } {
...
export function analyze(project: ProjectData): AnalysisResults {
  ...
  const slabDL =
    slabSelfWeightKPa(building.slabThicknessM) + building.finishesLoadKPa; // kPa
  ...
  for (let i = 0; i <= grid.nx; i++) {
    for (let j = 0; j <= grid.ny; j++) {
      ...
      const bw = building.beamWidthM,
        bh = building.beamDepthM;
      const beamW = beamSelfWeightKNperm(bw, bh);
      const Lx =
        (i === 0 ? grid.xSpacingsM[0] : grid.xSpacingsM[i - 1]) / 2 +
        (i === grid.nx ? grid.xSpacingsM[grid.nx - 1] : grid.xSpacingsM[i]) / 2;
      const Ly =
        (j === 0 ? grid.ySpacingsM[0] : grid.ySpacingsM[j - 1]) / 2 +
        (j === grid.ny ? grid.ySpacingsM[grid.ny - 1] : grid.ySpacingsM[j]) / 2;
      ...
      const pick = pickColumnSize(
        Pu,
        building.materials.fckMPa,
        building.materials.fyMPa,
      );
      const col: ColumnResult = {
        ...
        warnings:
          pick.phiPn < Pu
            ? ["Column axial capacity is less than required Pu"]
            : [],
      };
      columns.push(col);
    }
  ...
  for (let i = 0; i <= grid.nx; i++) {
    for (let j = 0; j < grid.ny; j++) {
      ...
      const tribWidth =
        (i === 0 ? grid.xSpacingsM[0] / 2 : grid.xSpacingsM[i - 1] / 2) +
        (i === grid.nx
          ? grid.xSpacingsM[grid.nx - 1] / 2
          : grid.xSpacingsM[i] / 2);
      const slabDL_per_m = slabDL * tribWidth; // kN/m
      const LL_per_m = LL * tribWidth; // kN/m
      const beamDL_per_m = beamSelfWeightKNperm(
  ...
  for (const c of columns) {
    ...
    const f: FootingResult = {
      ...
      suggestedSizeM: {
        b: sizeM,
        h: Math.max(0.4, Math.min(1.0, 0.5 + Pu / 5000)),
      },
      bearingPressureKPa: approxService / (sizeM * sizeM),
      warnings: [],
    };
    if (f.bearingPressureKPa > qAllow)
      f.warnings.push("Increase footing size (bearing pressure high)");
    footings.push(f);
  }
  ...
  for (const b of beams) {
    concreteM3 +=
      building.beamWidthM * building.beamDepthM * b.spanM * building.floors;
  }
  // slabs concrete
  ...
  for (const b of beams) {
    const Ast =
      (barAreasMM2[b.suggestedBars.diameterMM] * b.suggestedBars.count) / 1e6; // m2
    steelKg += Ast * b.spanM * building.floors * steelDensityKgPerM3;
  }
  for (const s of slabs) {
    const As_per_m =
      ((1000 / s.bar.spacingMM) * barAreasMM2[s.bar.diameterMM]) / 1e6; // m2 per m width
  ...
  const cost: CostSummary = {
    ...
    totalCost: round(
      concreteM3 * costs.concreteRatePerM3 + steelKg * costs.steelRatePerKg,
      0,
    ),
  };
...
export function seismicBNBC(project: ProjectData) {
  ...
  const h = building.floorHeightM * floors;
  const Ct =
    building.structuralSystem === "shear_wall"
      ? 0.05
      : building.structuralSystem === "rc_dual"
        ? 0.055
        : 0.075;
  const xExp = 0.75;
  ...
  const beamLen =
    sum(grid.xSpacingsM) * (grid.ny + 1) + sum(grid.ySpacingsM) * (grid.nx + 1);
  const D_per_floor_kN =
    slabDL * area +
    beamSelfWeightKNperm(building.beamWidthM, building.beamDepthM) * beamLen;
  const W = floors * (D_per_floor_kN + 0.25 * LL * area);
  // Site/seismic coefficients (simplified)
  const S_site =
    building.soil === "soft" ? 2.5 : building.soil === "stiff" ? 1.5 : 2.0;
  const Z = building.seismicZoneFactorZ ?? 0.2;
  const I = building.importanceFactorI ?? 1.0;
  const R = building.responseModificationR ?? 5.0;
  ...
  for (let k = 1; k <= floors; k++) {
    sum_wi_hi += wi * k * building.floorHeightM;
  }
  for (let k = 1; k <= floors; k++) {
    const hk = k * building.floorHeightM;
    const Fx = (V * (wi * hk)) / sum_wi_hi;
    storyForces.push({
      story: k,
      heightM: hk,
      weightKN: wi,
      lateralForceKN: round(Fx, 1),
      shearAboveKN: 0,
    });
  }
  let acc = 0;
  for (let i = floors - 1; i >= 0; i--) {
    acc += storyForces[i + 1]?.lateralForceKN ?? 0;
    storyForces[i].shearAboveKN = round(V - acc, 1);
  }

  return {
    periodT: round(T, 3),
    seismicWeightKN: round(W, 0),
    Cs: round(Cs, 3),
    baseShearKN: round(V, 0),
    storyForces,
  };
}
